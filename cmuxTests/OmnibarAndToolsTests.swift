...
final class FileExternalOpenTextTests: XCTestCase {
    func testCopyFileLabelIncludesLowercasedExtension() {
        let label = FileExternalOpenText.copyFileLabel(
            fileURL: URL(fileURLWithPath: "/tmp/Report.PDF")
        )

        XCTAssertEqual(label, "Copy .pdf File")
    }

    func testCopyFileLabelFallsBackWhenExtensionIsMissing() {
        let label = FileExternalOpenText.copyFileLabel(
            fileURL: URL(fileURLWithPath: "/tmp/README")
        )

        XCTAssertEqual(label, "Copy File")
    }
}
