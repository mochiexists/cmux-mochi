...
struct CLICodexHookTimeoutRegressionTests {
    ...
    @Test func codexInstalledAsyncStopDoesNotMarkNewerTurnIdle() throws {
        ...
        let transcriptURL = root.appendingPathComponent("codex-installed-stale-stop.jsonl")
        try FileManager.default.createDirectory(at: codexHome, withIntermediateDirectories: true)
    ...
    @Test func codexPromptSubmitDoesNotReviveStoppedTurn() throws {
        ...
        let transcriptURL = root.appendingPathComponent("codex-stale-session.jsonl")
    ...
    @Test func codexSessionStartRefreshesCompletedPriorTurn() throws {
        ...
        let stateURL = root.appendingPathComponent("codex-hook-sessions.json")
        let transcriptURL = root.appendingPathComponent("codex-fresh-session.jsonl")
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        try Data().write(to: transcriptURL)
        defer {
            Darwin.close(listenerFD)
