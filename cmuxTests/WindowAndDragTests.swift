...
final class FilePreviewPanelTextSavingTests: XCTestCase {
    ...
    private func closeWindow(_ window: NSWindow) {
        ...
        for scrollView in window.contentView?.subviews.compactMap({ $0 as? NSScrollView }) ?? [] {
    ...
    private func windowHosting(_ textView: NSTextView) -> NSWindow {
        if let previousScrollView = textView.enclosingScrollView {
            _ = textView.window?.makeFirstResponder(nil)
            previousScrollView.documentView = nil
        }
        let window = NSWindow(
        ...
        // ARC owns these windows through the test's explicit teardown.
