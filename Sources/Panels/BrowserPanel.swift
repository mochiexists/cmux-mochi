...
final class BrowserPanel: Panel, ObservableObject {
    ...
    /// Optional page-specific presentation mode for browser surfaces that host
    /// an existing web app but expose a narrower first-class cmux surface.
    @Published private(set) var contentMode: BrowserPanelContentMode
    ...
    private func configureNavigationDelegateCallbacks() {
        ...
        navigationDelegate.didFinish = { [weak self] webView in
            MainActor.assumeIsolated {
                ...
                self.applyContentModeAfterNavigation(reason: "navigationFinish")
            }
        }
    ...
    @discardableResult
    func setContentMode(_ mode: BrowserPanelContentMode) -> Bool {
        let changed = contentMode != mode
        guard changed else {
            applyContentModeAfterNavigation(reason: "contentModeRefresh")
            return false
        }
    ...
    func restoreSessionSnapshot(_ snapshot: SessionBrowserPanelSnapshot) {
        let restoredContentMode = snapshot.contentMode ?? .normal
        ...
        if let token = snapshot.diffViewerToken,
           ...
           let diffURL = CmuxDiffViewerURLSchemeHandler.diffViewerURL(token: token, requestPath: requestPath) {
            ...
            setOmnibarVisible(restoredContentMode.resolvedOmnibarVisible(snapshot.omnibarVisible, default: false))
            currentURL = diffURL
        ...
        hiddenWebViewDiscardManager.updateRestoredSessionRenderIntent(snapshot.shouldRenderWebView)
        setMuted(snapshot.isMuted)
        setOmnibarVisible(restoredContentMode.resolvedOmnibarVisible(snapshot.omnibarVisible, default: true))
    ...
    var sessionSnapshotContentMode: BrowserPanelContentMode? {
        contentMode.sessionSnapshotValue
    }
