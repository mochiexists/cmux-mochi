...
final class VSCodeServeWebWorkspaceRegistry {
    ...
    private let makeController: () -> VSCodeServeWebController
    private var controllersByWorkspaceID: [UUID: VSCodeServeWebController] = [:]

    private init(
        makeController: @escaping () -> VSCodeServeWebController = { VSCodeServeWebController() }
    ) {
    ...
    func ensureServeWebURL(
        ...
        controller(forWorkspaceID: workspaceID)
            .ensureServeWebURL(vscodeApplicationURL: vscodeApplicationURL, completion: completion)
    }

    func stop(workspaceID: UUID) {
        let controllers = queue.sync {
            controllersByWorkspaceID.removeValue(forKey: workspaceID).map { [$0] } ?? []
        }
    ...
    func stopAll() {
        let controllers = queue.sync {
            let controllers = Array(controllersByWorkspaceID.values)
            controllersByWorkspaceID.removeAll()
            return controllers
        }
        controllers.forEach { $0.stop() }
    }
    ...
    func restart(
        ...
        let controller = queue.sync {
            controllersByWorkspaceID[workspaceID]
        }
    ...
    func isServeWebURL(_ candidateURL: URL?) -> Bool {
        let controllers = queue.sync {
            Array(controllersByWorkspaceID.values)
        }
        return controllers.contains(where: { $0.isServeWebURL(candidateURL) })
    }

    private func controller(forWorkspaceID workspaceID: UUID) -> VSCodeServeWebController {
        queue.sync {
            if let controller = controllersByWorkspaceID[workspaceID] {
                return controller
            }
            let controller = makeController()
            controllersByWorkspaceID[workspaceID] = controller
            return controller
        }
    }
}
