...
BUNDLE_ID="com.cmux-mochi.staging"
BASE_APP_NAME="cmux"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
...
staging_slug_from_bundle_id() {
  local bundle_id="$1"
  local suffix=""
  if [[ "$bundle_id" == "com.cmux-mochi.staging."* ]]; then
    suffix="${bundle_id#com.cmux-mochi.staging.}"
  fi
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$BUNDLE_SET" -eq 0 ]]; then
    BUNDLE_ID="com.cmux-mochi.staging.${TAG_ID}"
  fi
