...
class Diagnostics:
    ...
    def app_pids(self) -> list[int]:
        ...
        if self.tag:
            patterns.append(f"cmux Mochi DEV {self.tag}.app/Contents/MacOS/cmux Mochi DEV")
            patterns.append(f"cmux Mochi DEV {self.tag}")
        patterns.append("cmux Mochi DEV")
        seen: set[int] = set()
...
def resolve_cli_path(raw: str | None, tag: str | None) -> str:
    ...
    if tag:
        candidates.append(os.path.expanduser(f"~/Library/Developer/Xcode/DerivedData/cmux-{tag}/Build/Products/Debug/cmux Mochi DEV {tag}.app/Contents/Resources/bin/cmux"))
