...
#
# Fork identity (app name, bundle id) comes from fork-identity.env, generated by
# scripts/inject-fork-identity.sh. Sourced here rather than left to the caller
# because this lib is sourced by scripts that do not source it themselves; a
# missing CMUX_FORK_BUNDLE_ID would silently write the pairing-host default to
# the wrong defaults domain, and the listener would never bind.
if [[ -z "${CMUX_FORK_BUNDLE_ID:-}" ]]; then
...
cmux_attach_mac_bundle_id() {
  printf '%s.debug.%s' "$CMUX_FORK_BUNDLE_ID" "$(cmux_attach__bundle_seg "$1")"
}
...
# sanitized slug, matching reload.sh (`APP_NAME="$CMUX_FORK_APP_NAME DEV ${TAG_SLUG}"`); the raw
# tag would miss for any tag whose slug differs (e.g. "Fix Foo" -> "fix-foo").
cmux_attach_mac_app_path() {
  local slug
  slug="$(cmux_attach__slug "$1")"
  printf '%s/Library/Developer/Xcode/DerivedData/cmux-%s/Build/Products/Debug/%s DEV %s.app' \
