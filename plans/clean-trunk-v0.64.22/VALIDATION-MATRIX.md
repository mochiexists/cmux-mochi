...
| `mobile.account-free-pairing` Account-free mobile pairing | DeviceLink and mobile packages; two signed-out QR UI selectors | Focused packages and both iPhone 17 simulator UI tests passed. | Cold-pair simulator and signed physical iPhone by Tailscale QR without Stack credentials. | DeviceLink v3 pairing and initial workspace sync passed on simulator and physical iPhone 17 Pro Max. |
| `mobile.reconnect-continuity` Mobile reconnect continuity | Focused mobile packages; `scripts/mobile-devicelink-e2e.sh` | Simulator and signed physical-device E2E passed. | Cold relaunch phone and Mac host after pairing, then verify identity adoption, reconnect, and workspace sync. | Both simulator and physical iPhone retained identity, reconnected, and resynchronized. |
...
| `navigation.sidebar-spring-load` Sidebar spring-load | `SidebarBonsplitWorkspaceSpringLoadTests` | Actual AppKit dragging-info, delay, cancellation, and target-change paths passed. | Route real `NSDraggingInfo` through the Bonsplit drop delegate and observe selection timing. | Candidate app-host journey passed; no human pointer-drag proof is claimed. |
