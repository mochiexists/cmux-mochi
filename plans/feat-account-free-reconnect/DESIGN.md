...
**Status:** design v2, not started. Revised after Codex round-1 review
(see `DESIGN_review_history.md`); awaiting round-2 consensus.
...
session-scoped. A photographed QR remains a ≤1 h exposure, not 30 days. Old iOS
clients see no change at all.
...
- `mobile.pairing.grant.register` — authorized by the live mac-scoped attach
  ...
  - Phone sends: a bounded device label (e.g. "Tim's iPhone 16", ≤64 chars).
  - Mac generates server-side: `grant_id`, `family_id`, a 32-byte secret
  ...
  - Mac persists **only a SHA-256 digest** of the secret, plus grant metadata
    (grant id, family id, device label, issue + expiry dates, pairing scope).
    The raw secret exists only in the response and then only in the phone's
    Keychain.
  - Multiple phones may register while the QR ticket lives; each registration
    is its own grant + family with its own label. (The QR is not single-use,
    matching today's behavior; what's per-phone is the grant.)
...
Exactly two verbs accept a grant, both new, both network-path:
...
2. Mac, atomically within the store mutation: revalidate the grant, mint the
   successor, persist `parent_digest + rotation_id → successor` in one commit.
   Persistence failure leaves the parent untouched and returns an error.
3. **Retrying the same `rotation_id` returns the same successor.** A spent
   parent is otherwise unusable (mint_session and fresh rotations fail).
...
- Reconnect flow, account-free branch: dial last-known Tailscale route →
  verify host identity via the **unauthenticated** `mobile.host.status` probe
  (macDeviceID + instanceTag must match the stored pairing) **before**
  presenting any credential → `mint_session` with the grant → attach with the
  fresh session ticket → opportunistic rotate (§4).
...
only a digest at rest on the Mac; revocable per family; rotation means a thief
racing the owner locks one of them out, which is at least *visible* (the
owner's next rotate fails against an unknown grant → the phone surfaces
"re-pair needed" and the Mac logs a security event). QR exposure stays ≤1 h.
