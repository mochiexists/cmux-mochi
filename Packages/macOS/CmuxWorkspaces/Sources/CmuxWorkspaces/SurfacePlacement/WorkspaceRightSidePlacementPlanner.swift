...
public struct WorkspaceRightSidePlacementPlanner: Sendable {
    ...
    /// Plans where to add a surface relative to `sourcePaneID`.
    public func plan(
        tree: ExternalTreeNode,
        sourcePaneID: UUID
    ) -> WorkspaceRightSidePlacement? {
        guard let source = sourcePane(in: tree, sourcePaneID: sourcePaneID) else {
            return nil
        }

        if let targetPaneID = nearestRightPane(to: source) {
            return .tab(targetPaneID: targetPaneID, strategy: .nearestRightSibling)
        }

        if source.ancestors.contains(where: { $0.split.orientation == "horizontal" }) {
            return .tab(targetPaneID: sourcePaneID, strategy: .sourceRightEdge)
        }

        return .splitRight(sourcePaneID: sourcePaneID, strategy: .fullWidthSplit)
    }
    ...
    private func sourcePane(
        ...
        case .split(let split):
            if var source = sourcePane(in: split.first, sourcePaneID: sourcePaneID) {
                source.ancestors.append(SplitAncestor(split: split, branch: .first))
                return source
            }
            if var source = sourcePane(in: split.second, sourcePaneID: sourcePaneID) {
    ...
    private func nearestRightPane(to source: SourcePane) -> UUID? {
        for ancestor in source.ancestors {
            ...
            if let targetPaneID = bestAlignedPane(
                to: source.pane,
                candidates: panes(in: ancestor.split.second)
            ) {
                return targetPaneID
            }
        }
    ...
    private func bestAlignedPane(
        ...
        let ranked = candidatesToRank.compactMap { pane -> RightPaneCandidate? in
            ...
            return RightPaneCandidate(
                ...
                verticalCenterDistance: abs(
                    pane.frame.y + (pane.frame.height / 2) - sourceCenterY
                )
            )
        }
