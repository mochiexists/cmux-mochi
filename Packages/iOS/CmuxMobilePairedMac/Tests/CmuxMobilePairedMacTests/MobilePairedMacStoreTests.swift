...
@Suite struct MobilePairedMacStoreTests {
    ...
    @Test func legacyGrantSchemaMigratesWithoutExposingRouteAuthority() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let databaseURL = directory.appendingPathComponent("paired-macs.sqlite3")
        let route = try tailscaleRoute(host: "100.64.0.20")

        try seedVersionSevenDatabase(at: databaseURL, routes: [route])
        let store = try MobilePairedMacStore(databaseURL: databaseURL)
        let mac = try #require(await store.activeMac(stackUserID: "user-1"))

        #expect(mac.routes == [route])
    }
