...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/cmux Mochi DEV.app")
...
def _find_app():
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/cmux-{preferred_slug}/Build/Products/Debug/cmux Mochi DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/cmux-{preferred_slug}/Build/Products/Debug/cmux Mochi DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/cmux Mochi DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/cmux-*/Build/Products/Debug/cmux Mochi DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/cmux-*/Build/Products/Debug/cmux Mochi DEV*.app"))
...
def _kill_cmux(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/cmux Mochi DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "cmux Mochi DEV"], capture_output=True)
    time.sleep(1.5)
...
def run_tests():
    ...
    app_path = _find_app()
    if not app_path:
        print("Error: Could not find cmux Mochi DEV.app in DerivedData")
        return 1
