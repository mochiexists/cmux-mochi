    ...
    with tempfile.TemporaryDirectory(prefix="cmux-session-relaunch-agents-") as td:
        ...
        try:
            ...
            try:
                ...
                def workspace_screen(index: int) -> str:
                    if len(client.list_workspaces()) <= index:
                        return ""
                    client.select_workspace(index)
                    return _read_scrollback(client)

                for index, session_id, echo_prefix in resume_checks:
                    # 1) the resume command must be PRE-TYPED on the restored pane
                    #    (the session id is visible on the input line).
                    pre_typed = _wait_for_condition(
                        12.0, lambda i=index, s=session_id: s in workspace_screen(i)
                    )
                    screen = workspace_screen(index)
                    if not pre_typed:
                ...
                # Scrollback regression: the Codex pane (workspace 0) must replay
                # its prior active-TUI scrollback on reopen, not just the pre-typed
                # resume line.
                if not _wait_for_condition(
                    12.0, lambda: scrollback_marker in workspace_screen(0)
                ):
                    tail = "\n".join(workspace_screen(0).splitlines()[-20:])
                    failures.append(
